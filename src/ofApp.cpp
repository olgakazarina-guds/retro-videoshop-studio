...
void ofApp::update() {
    ...
    // Keep rotation controls in the lower camera rail so they never cover a
    // title, intensity label, or media preview.
    if (currentState != AppState::HOME) {
        ...
        // Quad View uses a slightly lower rail because its panels extend
        // farther down than the single-preview screens.
